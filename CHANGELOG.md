...
- Fail-closed soft-OOB: storms never ghost-place; MESH_EXCHANGE C(4,2)=6
- Energy path: outer-segment → cilium → ellipsoid → myoid → nucleus → outer fiber → spherule → inner fiber → pedicle invaginating cone triad life-force seal
- Usability: cubalc_smx_pedicle_invaginating_ready() pure-C face
...
- Proofs: 99_smx_rpe + 1850_smx_mesh_rpe_life = 60/0; flagship 1620 = 12/0 PASS
- c_slice=src/cubalc_smx_rpe.c
...
- Energy path: basilar trunk apex → superior cerebellar takeoff → ambient cistern arc → tentorial notch → cerebellar hemispheric / vermian perfusion seal · midbrain tegmentum feed
- C slice: src/cubalc_smx_superior_cerebellar.c · WE_AUTOHEAL under locked rails

## 1.15.1315-usability — 1837 MESH_SUPERIOR_CEREBELLAR_LIFE
- SMX soft-OOB fail-closed mesh stability + life-force superior cerebellar conduit (basilar trunk → SCA takeoff → cerebellar hemisphere / midbrain tegmentum → tentorial notch seal).
- Proofs: 1837+99 PASS 60/0; flagship 1620 PASS 12/0.
...
- Pure-C src/cubalc_smx_left_subclavian.c + proofs 99/1833 (60/0 PASS)
- Flagship 1620: PASS (12/0)
- Energy path: aortic arch apex → left subclavian origin → left subclavian trunk → axillary takeoff → upper limb / vertebral perfusion
