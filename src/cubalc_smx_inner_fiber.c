 ...
 * ellipsoid → myoid → nucleus soma → outer fiber axon hillock → inner_fiber synaptic terminal
 * ribbon synapse life-force seal · inner_fiber mesh.
 * Pure C. No SYS glue. Usability: named SMX face for photoreceptor rod inner_fiber.
 */
#include <string.h>
...
int cubalc_smx_inner_fiber_ribbon_ready(void) {
  return 1;
}
...
int cubalc_smx_inner_fiber_selftest(void) {
  ...
  if (cubalc_smx_inner_fiber_ribbon_ready() != 1) return 0;
  return 1;
}
