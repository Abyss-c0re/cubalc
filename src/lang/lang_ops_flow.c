...
int cubalc_lang_ops_flow(VM *vm, Lex *L){
  ...
  /* SENDANY|TRYSENDANY obj methods… [WITH args…] [OR|DEFAULT fb]
   * Soft TRYSENDANY: OK=0 if obj/method miss (no OR). Hard SENDANY: fail-fast.
   * … OR fb → miss/soft arity → LAST=fb, OK=1, SENDANY_OR=1 (CALLANY OR twin).
   * Usability: multi-method hooks with default without HASMETHODANY+IF+SEND glue.
   * Twin of CALLANY OR for method plane. 0-arity when no WITH. */
  if (kw(&L->cur, "SENDANY") || kw(&L->cur, "SEND_ANY") ||
    ...
    if (nname == 0) {
      ...
      fail_at(vm, L, "SENDANY obj methods… [WITH args] [OR fb] — SENDANY obj tick update");
      return -1;
    }
    ob = oop_find_obj(vm, oname);
    if (!ob) {
      ...
      }
      {
        ...
        if (or_rc > 0) {
          ...
          var_set_str(vm, "ERR", "");
          if (vm->trace)
            fprintf(vm->trace, "# SENDANY OR fallback after obj miss %s\n", oname);
          bump(vm);
          return 1;
        }
      }
      ...
      }
      {
        ...
        }
        {
          ...
          if (or_rc > 0) {
            ...
            var_set_str(vm, "ERR", "");
            if (vm->trace)
              fprintf(vm->trace, "# SENDANY OR fallback after method miss %s\n",
                      ebuf[0] ? ebuf : "?");
            bump(vm);
            return 1;
          }
    ...
    /* discard unused OR after successful SENDANY */
    {
      char fb[256];
      int or_rc = call_parse_or_fallback(vm, L, fb, sizeof fb);
      if (or_rc < 0) return -1;
  ...
  /* HASCLASSANY|NEEDCLASSANY name… — multi CLASS any-of gate (HASCLASSES twin).
   * Soft: LAST_N 1 if any CLASS defined · CLASSHAVE bag · CLASSMISS bag.
   * NEEDCLASSANY fail-fast if none of the listed classes exist.
   * Usability: optional plugin class OR without N× HASCLASS IF glue. */
  if (kw(&L->cur, "HASCLASSANY") || kw(&L->cur, "HAS_CLASS_ANY") ||
      kw(&L->cur, "ANYCLASS") || kw(&L->cur, "CLASSANY") ||
      kw(&L->cur, "HASANYCLASS") || kw(&L->cur, "NEEDCLASSANY") ||
      kw(&L->cur, "NEED_CLASS_ANY") || kw(&L->cur, "REQUIRECLASSANY") ||
      kw(&L->cur, "MUSTCLASSANY")) {
    int hard = kw(&L->cur, "NEEDCLASSANY") || kw(&L->cur, "NEED_CLASS_ANY") ||
               kw(&L->cur, "REQUIRECLASSANY") || kw(&L->cur, "MUSTCLASSANY");
    char names[32][48];
    ...
    while (nname < 32 &&
      ...
      if (L->cur.kind == TK_IDENT &&
          (kw(&L->cur, "ASSERT") || kw(&L->cur, "LET") || kw(&L->cur, "PRINT") ||
           ...
           kw(&L->cur, "CUBE") || kw(&L->cur, "PLUG") || kw(&L->cur, "HOLD_FLASH") ||
           kw(&L->cur, "CLASS") || kw(&L->cur, "NEW") || kw(&L->cur, "HASCLASS") ||
           kw(&L->cur, "HASCLASSES") || kw(&L->cur, "LISTCLASSES") ||
           kw(&L->cur, "HASFORM") || oop_stmt_kw(L)))
        break;
      if (L->cur.kind == TK_STR) {
        const char *s = L->cur.text;
        if (strchr(s, '\n') || strchr(s, ',') || strchr(s, ' ')) {
    ...
    if (nname == 0) {
      if (hard) {
        fail_at(vm, L, "NEEDCLASSANY name… — NEEDCLASSANY Ticket Cell · HASCLASSANY bag");
        return -1;
      }
      var_set_str(vm, "LAST", "0");
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      vm->last_n = 0;
      var_set_num(vm, "LAST_N", 0);
      var_set_num(vm, "HASCLASSANY_N", 0);
      var_set_num(vm, "CLASSMISS_N", 0);
      var_set_num(vm, "CLASSHAVE_N", 0);
      var_set_str(vm, "CLASSMISS", "");
      var_set_str(vm, "CLASSHAVE", "");
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASCLASSANY: need names — HASCLASSANY Ticket Ghost");
      var_set_str(vm, "ERR", "HASCLASSANY: need names — HASCLASSANY Ticket Ghost");
      bump(vm);
      return 1;
    }
    for (i = 0; i < nname; i++) {
    ...
    var_set_str(vm, "CLASSMISS", miss);
    var_set_str(vm, "CLASSHAVE", havebag);
    var_set_num(vm, "CLASSMISS_N", nmiss);
    var_set_num(vm, "CLASSHAVE_N", nhave);
    var_set_num(vm, "HASCLASSANY_N", nhave);
    var_set_num(vm, "NEEDCLASSANY_N", nname);
    var_set_num(vm, "CLASSES_ANY_N", nname);
    if (nhave > 0) {
      var_set_num(vm, "LAST_N", 1);
      vm->last_n = 1;
      var_set_str(vm, "LAST", "1");
      ...
      char em[240];
      snprintf(em, sizeof em,
               "NEEDCLASSANY miss line %d: need one of [%s] — CLASS/INCLUDE · HASCLASSANY · LISTCLASSES",
               aln, miss[0] ? miss : "?");
      var_set_num(vm, "LAST_N", 0);
      vm->last_n = 0;
      fail_at(vm, L, em);
      ...
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      snprintf(em, sizeof em,
               "HASCLASSANY miss (0/%d): need one of %s — NEEDCLASSANY · INCLUDE",
               nname, miss[0] ? miss : "?");
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", em);
      var_set_str(vm, "ERR", em);
    ...
    if (vm->trace)
      fprintf(vm->trace, "# %s n=%d have=%d miss=%d\n",
              hard ? "NEEDCLASSANY" : "HASCLASSANY", nname, nhave, nmiss);
    bump(vm);
    return 1;
  }
  ...
  if (kw(&L->cur, "HASMETHODS") || kw(&L->cur, "HAS_METHODS") ||
    ...
    if (nname == 0) {
      ...
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASMETHODS: need methods — HASMETHODS Cell tick init");
      var_set_str(vm, "ERR", "HASMETHODS: need methods — HASMETHODS Cell tick init");
      bump(vm);
      return 1;
    }
    ...
    for (i = 0; i < nname; i++) {
      int hit = 0;
      if (cd && oop_find_method(cd, names[i]))
        hit = 1;
      if (hit)
        continue;
      nmiss++;
      ...
      if (hard) {
        char em[220];
        snprintf(em, sizeof em,
                 "NEEDMETHODS miss line %d: '%s.%s' (+%d more?) — CLASS METHOD · HASMETHOD · LISTMETHODS",
                 aln, target[0] ? target : "?", names[i], nname - i - 1);
        fail_at(vm, L, em);
        return -1;
      }
    }
    ...
    if (nmiss == 0) {
      ...
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      snprintf(em, sizeof em,
               "HASMETHODS miss (%d/%d) on %s: %s — CLASS METHOD · NEEDMETHODS · LISTMETHODS",
               nmiss, nname, target[0] ? target : "?", miss);
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", em);
      var_set_str(vm, "ERR", em);
    }
    if (vm->trace)
      fprintf(vm->trace, "# %s %s n=%d miss=%d\n",
              hard ? "NEEDMETHODS" : "HASMETHODS", target, nname, nmiss);
    bump(vm);
    return 1;
  }
  ...
  if (kw(&L->cur, "HASMETHODANY") || kw(&L->cur, "HAS_METHOD_ANY") ||
      ...
      kw(&L->cur, "NEED_METHOD_ANY") || kw(&L->cur, "REQUIREMETHODANY") ||
    ...
    if (nname == 0) {
      ...
      var_set_str(vm, "METHOD_ON", target);
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASMETHODANY: need methods — HASMETHODANY Cell tick ghost");
      var_set_str(vm, "ERR", "HASMETHODANY: need methods — HASMETHODANY Cell tick ghost");
      bump(vm);
      return 1;
    }
    ob = oop_find_obj(vm, target);
    ...
    if (nhave > 0) {
      ...
      char em[240];
      snprintf(em, sizeof em,
               "NEEDMETHODANY miss line %d: need one of %s.%s — CLASS METHOD · HASMETHODANY · LISTMETHODS",
      ...
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      snprintf(em, sizeof em,
               "HASMETHODANY miss (0/%d) on %s: need one of %s — NEEDMETHODANY · LISTMETHODS",
    ...
    if (vm->trace)
      fprintf(vm->trace, "# %s %s n=%d have=%d miss=%d\n",
              hard ? "NEEDMETHODANY" : "HASMETHODANY", target, nname, nhave, nmiss);
    bump(vm);
    return 1;
  }
...
/* HASOBJ|LIVES|ISOBJ|ALIVEOBJ name — soft probe if live object exists.
  ...
  /* HASFIELDS|NEEDFIELDS Class|obj field… | Class.field bag —
   * multi field capability gate (all-or-FIELDMISS).
   * Soft: LAST_N 0|1 · FIELDMISS/FLDMISS bag · NEEDFIELDS fail-fast first miss.
   * Forms: HASFIELDS Cell x y · HASFIELDS bag_of_Class.field (multi-class).
   * Usability: agent GETF/SETF / field_guard contracts without N× HASFIELD soup. */
  if (kw(&L->cur, "HASFIELDS") || kw(&L->cur, "HAS_FIELDS") ||
      kw(&L->cur, "ALLFIELDS") || kw(&L->cur, "FIELDSOK") ||
      kw(&L->cur, "NEEDFIELDS") || kw(&L->cur, "NEED_FIELDS") ||
      kw(&L->cur, "REQUIREFIELDS") || kw(&L->cur, "MUSTFIELDS")) {
    int hard = kw(&L->cur, "NEEDFIELDS") || kw(&L->cur, "NEED_FIELDS") ||
               kw(&L->cur, "REQUIREFIELDS") || kw(&L->cur, "MUSTFIELDS");
    char first[1024];
    char target[48];
    char names[32][48];
    ...
    if (L->cur.kind != TK_IDENT && L->cur.kind != TK_STR) {
      if (hard) {
        fail_at(vm, L, "NEEDFIELDS Class|obj field… — NEEDFIELDS Cell x · bag Class.field");
        return -1;
      }
      var_set_str(vm, "LAST", "0");
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      vm->last_n = 0;
      var_set_num(vm, "LAST_N", 0);
      var_set_num(vm, "HASFIELDS_N", 0);
      var_set_num(vm, "FIELDMISS_N", 0);
      var_set_num(vm, "FLDMISS_N", 0);
      var_set_str(vm, "FIELDMISS", "");
      var_set_str(vm, "FLDMISS", "");
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASFIELDS: need Class|obj field… or Class.field bag");
      var_set_str(vm, "ERR", "HASFIELDS: need Class|obj field… or Class.field bag");
      bump(vm);
      return 1;
    }
    if (L->cur.kind == TK_STR) {
    ...
    if (dotted) {
      ...
      if (nname == 0) {
        if (hard) {
          fail_at(vm, L, "NEEDFIELDS Class.field… — NEEDFIELDS Ticket.id Order.n");
          return -1;
        }
        var_set_str(vm, "LAST", "0");
        snprintf(vm->last_str, sizeof vm->last_str, "0");
        ...
        var_set_str(vm, "FIELDMISS", "");
        var_set_str(vm, "FLDMISS", "");
        var_set_str(vm, "FIELD_ON", "*");
        var_set_num(vm, "OK", 0);
        var_set_str(vm, "LAST_ERR", "HASFIELDS: need Class.field bag");
        var_set_str(vm, "ERR", "HASFIELDS: need Class.field bag");
        bump(vm);
        return 1;
      }
      for (i = 0; i < nname; i++) {
        char cname[48], fname[48];
        const char *dot = strchr(names[i], '.');
        int hit = 0;
        cname[0] = 0;
        fname[0] = 0;
        if (dot && dot != names[i] && dot[1]) {
          ...
          snprintf(fname, sizeof fname, "%s", dot + 1);
          ob = oop_find_obj(vm, cname);
          if (ob)
            cd = &vm->classes[ob->class_idx];
          else
            cd = oop_find_class(vm, cname);
          if (cd && oop_field_idx(cd, fname) >= 0)
            hit = 1;
        }
        if (hit)
          continue;
        ...
        if (hard) {
          char em[220];
          snprintf(em, sizeof em,
                   "NEEDFIELDS miss line %d: '%s' (+%d more?) — CLASS FIELD · HASFIELD · LISTFIELDS",
                   aln, names[i], nname - i - 1);
          fail_at(vm, L, em);
          return -1;
        }
      ...
      var_set_num(vm, "HASFIELDS_N", nname - nmiss);
      var_set_num(vm, "NEEDFIELDS_N", nname);
      var_set_num(vm, "FIELDS_N", nname);
      var_set_str(vm, "FIELD_ON", "*");
      if (nmiss == 0) {
        var_set_num(vm, "LAST_N", 1);
        vm->last_n = 1;
        ...
        snprintf(vm->last_str, sizeof vm->last_str, "0");
        snprintf(em, sizeof em,
                 "HASFIELDS miss (%d/%d): %s — CLASS FIELD · NEEDFIELDS · LISTFIELDS",
                 nmiss, nname, miss);
        var_set_num(vm, "OK", 0);
        var_set_str(vm, "LAST_ERR", em);
        var_set_str(vm, "ERR", em);
      ...
      if (vm->trace)
        fprintf(vm->trace, "# %s dotted n=%d miss=%d\n",
                hard ? "NEEDFIELDS" : "HASFIELDS", nname, nmiss);
      bump(vm);
      return 1;
    }
    /* classic Class|obj field… form */
    snprintf(target, sizeof target, "%s", first);
    while (nname < 32 &&
           (L->cur.kind == TK_STR || L->cur.kind == TK_IDENT)) {
      if (L->cur.kind == TK_IDENT &&
    ...
    if (nname == 0) {
      if (hard) {
        fail_at(vm, L, "NEEDFIELDS Class|obj field… — NEEDFIELDS Cell x y");
        return -1;
      }
      var_set_str(vm, "LAST", "0");
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      ...
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASFIELDS: need fields — HASFIELDS Cell x y");
      var_set_str(vm, "ERR", "HASFIELDS: need fields — HASFIELDS Cell x y");
      bump(vm);
      return 1;
    }
    ob = oop_find_obj(vm, target);
    ...
    for (i = 0; i < nname; i++) {
      ...
      if (hard) {
        char em[220];
        snprintf(em, sizeof em,
                 "NEEDFIELDS miss line %d: '%s.%s' (+%d more?) — CLASS FIELD · HASFIELD · LISTFIELDS",
                 aln, target[0] ? target : "?", names[i], nname - i - 1);
        fail_at(vm, L, em);
        return -1;
      }
    ...
    if (nmiss == 0) {
      ...
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      snprintf(em, sizeof em,
               "HASFIELDS miss (%d/%d) on %s: %s — CLASS FIELD · NEEDFIELDS · LISTFIELDS",
               nmiss, nname, target[0] ? target : "?", miss);
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", em);
      var_set_str(vm, "ERR", em);
  ...
  /* HASFIELDANY|NEEDFIELDANY Class|obj field… | Class.field bag —
   * multi field any-of gate (HASFIELDS twin).
   * Soft: LAST_N 1 if any match · FIELDHAVE/FLDHAVE · FIELDMISS/FLDMISS.
   * Forms: HASFIELDANY Cell alive · HASFIELDANY bag_of_Class.field.
   * NEEDFIELDANY fail-fast if none match. */
  if (kw(&L->cur, "HASFIELDANY") || kw(&L->cur, "HAS_FIELD_ANY") ||
    ...
    if (L->cur.kind != TK_IDENT && L->cur.kind != TK_STR) {
      if (hard) {
        fail_at(vm, L, "NEEDFIELDANY Class|obj field… — NEEDFIELDANY Cell alive · bag Class.field");
        return -1;
      }
      var_set_str(vm, "LAST", "0");
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      ...
      var_set_num(vm, "FLDMISS_N", 0);
      var_set_num(vm, "FLDHAVE_N", 0);
      var_set_str(vm, "FIELDMISS", "");
      var_set_str(vm, "FIELDHAVE", "");
      var_set_str(vm, "FLDMISS", "");
      var_set_str(vm, "FLDHAVE", "");
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASFIELDANY: need Class|obj field… or Class.field bag");
      var_set_str(vm, "ERR", "HASFIELDANY: need Class|obj field… or Class.field bag");
      bump(vm);
      return 1;
    }
    if (L->cur.kind == TK_STR) {
    ...
    if (dotted) {
      ...
      if (nname == 0) {
        if (hard) {
          fail_at(vm, L, "NEEDFIELDANY Class.field… — NEEDFIELDANY Ticket.id Order.n");
          return -1;
        }
        var_set_str(vm, "LAST", "0");
        snprintf(vm->last_str, sizeof vm->last_str, "0");
        ...
        var_set_str(vm, "FIELD_ON", "*");
        var_set_num(vm, "OK", 0);
        var_set_str(vm, "LAST_ERR", "HASFIELDANY: need Class.field bag");
        var_set_str(vm, "ERR", "HASFIELDANY: need Class.field bag");
        bump(vm);
        return 1;
      }
      for (i = 0; i < nname; i++) {
      ...
      if (nhave > 0) {
        ...
        char em[240];
        snprintf(em, sizeof em,
                 "NEEDFIELDANY miss line %d: need one of [%s] — CLASS FIELD · HASFIELDANY · LISTFIELDS",
                 aln, miss[0] ? miss : "?");
        var_set_num(vm, "LAST_N", 0);
        vm->last_n = 0;
        fail_at(vm, L, em);
        ...
        snprintf(vm->last_str, sizeof vm->last_str, "0");
        snprintf(em, sizeof em,
                 "HASFIELDANY miss (0/%d): need one of %s — NEEDFIELDANY · LISTFIELDS",
                 nname, miss[0] ? miss : "?");
        var_set_num(vm, "OK", 0);
        var_set_str(vm, "LAST_ERR", em);
        var_set_str(vm, "ERR", em);
      ...
      if (vm->trace)
        fprintf(vm->trace, "# %s dotted n=%d have=%d miss=%d\n",
                hard ? "NEEDFIELDANY" : "HASFIELDANY", nname, nhave, nmiss);
      bump(vm);
      return 1;
    }
    /* classic Class|obj field… form */
  ...
  /* HASFNS|NEEDFNS name… — multi FN capability gate (all-or-FNMISS).
   * Soft: LAST_N 0|1 · FNMISS bag · NEEDFNS fail-fast first miss.
   * Usability: agent CALL contracts without N× HASFN glue · bag/CSV/idents.
   * Twin of HASOBJS for FN plane · dual of single HASFN. */
  if (kw(&L->cur, "HASFNS") || kw(&L->cur, "HAS_FNS") ||
      kw(&L->cur, "ALLFNS") || kw(&L->cur, "FNSOK") ||
      kw(&L->cur, "HASFUNCS") || kw(&L->cur, "ALLFUNCS") ||
      kw(&L->cur, "NEEDFNS") || kw(&L->cur, "NEED_FNS") ||
      kw(&L->cur, "REQUIREFNS") || kw(&L->cur, "MUSTFNS") ||
      kw(&L->cur, "NEEDFUNCS") || kw(&L->cur, "REQUIREFUNCS")) {
    int hard = kw(&L->cur, "NEEDFNS") || kw(&L->cur, "NEED_FNS") ||
               kw(&L->cur, "REQUIREFNS") || kw(&L->cur, "MUSTFNS") ||
               kw(&L->cur, "NEEDFUNCS") || kw(&L->cur, "REQUIREFUNCS");
    char names[32][48];
    char miss[1024];
    int nname = 0, nmiss = 0, i, j, aln = L->cur.line;
    size_t mo = 0;
    lex_next(L);
    miss[0] = 0;
    if (kw(&L->cur, "FN") || kw(&L->cur, "FUNC") || kw(&L->cur, "FUNCTION") ||
    ...
    if (nname == 0) {
      if (hard) {
        fail_at(vm, L, "NEEDFNS name… — NEEDFNS add greet · HASFNS bag");
        return -1;
      }
      var_set_str(vm, "LAST", "0");
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      vm->last_n = 0;
      var_set_num(vm, "LAST_N", 0);
      var_set_num(vm, "HASFNS_N", 0);
      var_set_num(vm, "FNMISS_N", 0);
      var_set_str(vm, "FNMISS", "");
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", "HASFNS: need names — HASFNS add greet");
      var_set_str(vm, "ERR", "HASFNS: need names — HASFNS add greet");
      bump(vm);
      return 1;
    }
    for (i = 0; i < nname; i++) {
      ...
      if (hard) {
        char em[220];
        snprintf(em, sizeof em,
                 "NEEDFNS miss line %d: '%s' (+%d more?) — DEFINE/INCLUDE · HASFN · LISTFNS",
                 aln, names[i], nname - i - 1);
        fail_at(vm, L, em);
        return -1;
      }
    ...
    var_set_num(vm, "HASFNS_N", nname - nmiss);
    var_set_num(vm, "NEEDFNS_N", nname);
    var_set_num(vm, "FNS_N", nname);
    if (nmiss == 0) {
      var_set_num(vm, "LAST_N", 1);
      vm->last_n = 1;
      var_set_str(vm, "LAST", "1");
      ...
      snprintf(vm->last_str, sizeof vm->last_str, "0");
      snprintf(em, sizeof em,
               "HASFNS miss (%d/%d): %s — DEFINE/INCLUDE · NEEDFNS · LISTFNS",
               nmiss, nname, miss);
      var_set_num(vm, "OK", 0);
      var_set_str(vm, "LAST_ERR", em);
      var_set_str(vm, "ERR", em);
    ...
    if (vm->trace)
      fprintf(vm->trace, "# %s n=%d miss=%d\n",
              hard ? "NEEDFNS" : "HASFNS", nname, nmiss);
    bump(vm);
    return 1;
  }
  ...
  if (kw(&L->cur,"EACH")||kw(&L->cur,"FOREACH")){
    ...
    int is_meth = (kw(&L->cur,"METHOD")||kw(&L->cur,"METHODS")||kw(&L->cur,"METH")||
                   kw(&L->cur,"MSG")||kw(&L->cur,"MSGS")||kw(&L->cur,"MESSAGE")||
                   kw(&L->cur,"MESSAGES")||kw(&L->cur,"HANDLER")||kw(&L->cur,"HANDLERS")||
                   kw(&L->cur,"OBJMETHOD")||kw(&L->cur,"OBJMETHODS")||
                   kw(&L->cur,"CLASSMETHOD")||kw(&L->cur,"CLASSMETHODS"));
    ...
    if (is_meth){
      /* EACH METHOD|MSG|HANDLER OF obj|Class [AS name] ... END
       * Binds each CLASS method name to name (default METHOD); IT=0-based; METHOD_N=1-based.
       * OF required: live object or CLASS name. Soft empty if unknown.
       * Usability: no LISTMETHODS+EACH LINE glue before SEND/TRYSEND
       * (twin of EACH PROP; METHOD/THIS). */
      char bind[48], target[48];
      ClassDef *cd = NULL;
      ObjInst *ob = NULL;
      int i, n = 0;
      snprintf(bind, sizeof bind, "METHOD");
      target[0] = 0;
      if (kw(&L->cur, "AS") || kw(&L->cur, "->")) {
        lex_next(L);
        if (L->cur.kind != TK_IDENT) { fail(vm, "EACH METHOD as name"); return -1; }
        snprintf(bind, sizeof bind, "%s", L->cur.text);
        lex_next(L);
      } else if (L->cur.kind == TK_IDENT && !kw(&L->cur, "OF") &&
                 !kw(&L->cur, "ON") && !kw(&L->cur, "IN") &&
      ...
      if (strcasecmp(L->cur.text, "THIS") == 0 ||
        ...
        fail(vm, "EACH METHOD OF obj|Class"); return -1;
      }
      if (target[0]) {
        ob = oop_find_obj(vm, target);
        if (ob && ob->class_idx >= 0 && ob->class_idx < vm->n_classes)
      ...
      skip_nl(L);
      {
        ...
        if (depth != 0) { fail(vm, "EACH METHOD without END"); return -1; }
        if (cd) {
          for (i = 0; i < cd->n_methods && !vm->fatal && !vm->halt; i++) {
            const char *mname = cd->methods[i].name;
            var_set_str(vm, bind, mname);
            var_set_str(vm, "METHOD", mname);
            var_set_str(vm, "MSG", mname);
            var_set_str(vm, "HANDLER", mname);
            var_set_str(vm, "CLASS", cd->name);
            if (ob) {
              var_set_str(vm, "OBJECT", ob->name);
              var_set_str(vm, "OBJ", ob->name);
            ...
            var_set_num(vm, "IT", n);
            var_set_num(vm, "IDX", n);
            var_set_num(vm, "METHOD_N", n + 1);
            var_set_num(vm, "MSG_N", n + 1);
            var_set_num(vm, "OK", 1);
            vm->break_loop = 0;
            vm->continue_loop = 0;
            {
        ...
        var_set_num(vm, "OK", cd ? 1 : 0);
        if (!cd) {
          var_set_str(vm, "LAST_ERR", "EACH METHOD: unknown obj/class");
          var_set_str(vm, "ERR", "EACH METHOD: unknown obj/class");
        }
        bump(vm);
        return 1;
      }
  ...
  if (kw(&L->cur,"TRY")||kw(&L->cur,"GUARD")||kw(&L->cur,"WITHCLEANUP")||
    ...
    /* run try body until CATCH|FINALLY|END (or halt/fatal) */
    {
      ...
      while (!vm->fatal && !vm->halt) {
        ...
        {
          int r = parse_form(vm, &body);
          if (r < 0) { body_rc = -1; break; }
          if (r == 0) break;
        }
        if (vm->break_loop || vm->continue_loop || vm->return_fn || vm->halt) break;
      }
      if (vm->fatal) body_rc = -1;
    }
    saved_halt = vm->halt;
    saved_exit = vm->exit_code;
    saved_return = vm->return_fn;
    ...
    if (saved_halt) {
      vm->halt = 1;
      vm->exit_code = saved_exit;
    }
    if (saved_return) vm->return_fn = 1;
   ...
   * IFSTARTSI/IFENDSI/IFCONTAINSI/IFEQSI — case-insensitive twins (strncasecmp).
   * Usability: path/log/role branches without UPPER+probe + IF LAST_N. */
  if (kw(&L->cur,"IFSTARTS")||kw(&L->cur,"WHENSTARTS")||kw(&L->cur,"IFPREFIX")||
      kw(&L->cur,"IFSTARTSI")||kw(&L->cur,"WHENSTARTSI")||kw(&L->cur,"IFPREFIXI")||
      kw(&L->cur,"IFENDS")||kw(&L->cur,"WHENENDS")||kw(&L->cur,"IFSUFFIX")||
      kw(&L->cur,"IFENDSI")||kw(&L->cur,"WHENENDSI")||kw(&L->cur,"IFSUFFIXI")||
      kw(&L->cur,"IFCONTAINS")||kw(&L->cur,"WHENCONTAINS")||kw(&L->cur,"IFHAS")||
      kw(&L->cur,"WHENHAS")||kw(&L->cur,"IFINCLUDES")||
      kw(&L->cur,"IFCONTAINSI")||kw(&L->cur,"WHENCONTAINSI")||kw(&L->cur,"IFHASI")||
      kw(&L->cur,"WHENHASI")||kw(&L->cur,"IFINCLUDESI")||
      kw(&L->cur,"IFEQS")||kw(&L->cur,"IFSTARTSI")||kw(&L->cur,"IFENDSI")||kw(&L->cur,"IFCONTAINSI")||kw(&L->cur,"IFHASI")||kw(&L->cur,"IFEQSI")||kw(&L->cur,"WHENEQS")||kw(&L->cur,"IFSTREQ")||
      kw(&L->cur,"IFEQUALSTR")||
      kw(&L->cur,"IFEQSI")||kw(&L->cur,"WHENEQSI")||kw(&L->cur,"IFSTREQI")||
      kw(&L->cur,"IFEQUALSTRI")){
    int aln = L->cur.line;
    ...
    if (strcmp(op, "IFENDS") == 0 || strcmp(op, "WHENENDS") == 0 ||
        ...
        strcmp(op, "IFENDSI") == 0 || strcmp(op, "WHENENDSI") == 0 ||
        strcmp(op, "IFSUFFIXI") == 0)
      mode = 1;
    else if (strcmp(op, "IFCONTAINS") == 0 || strcmp(op, "WHENCONTAINS") == 0 ||
             strcmp(op, "IFHAS") == 0 || strcmp(op, "WHENHAS") == 0 ||
             strcmp(op, "IFINCLUDES") == 0 ||
             strcmp(op, "IFCONTAINSI") == 0 || strcmp(op, "WHENCONTAINSI") == 0 ||
             strcmp(op, "IFHASI") == 0 || strcmp(op, "WHENHASI") == 0 ||
             strcmp(op, "IFINCLUDESI") == 0)
      mode = 2;
    else if (strcmp(op, "IFEQS") == 0 || strcmp(op, "WHENEQS") == 0 ||
             strcmp(op, "IFSTREQ") == 0 || strcmp(op, "IFEQUALSTR") == 0 ||
             strcmp(op, "IFEQSI") == 0 || strcmp(op, "WHENEQSI") == 0 ||
             strcmp(op, "IFSTREQI") == 0 || strcmp(op, "IFEQUALSTRI") == 0)
      mode = 3;
    else
      mode = 0;
    if (op[strlen(op) - 1] == 'I' && strcmp(op, "IFHAS") != 0 &&
        strcmp(op, "WHENHAS") != 0) {
      /* trailing I = case-insensitive family (not IFHAS/WHENHAS) */
      if (strcmp(op, "IFSTARTSI") == 0 || strcmp(op, "WHENSTARTSI") == 0 ||
          strcmp(op, "IFPREFIXI") == 0 ||
          strcmp(op, "IFENDSI") == 0 || strcmp(op, "WHENENDSI") == 0 ||
          strcmp(op, "IFSUFFIXI") == 0 ||
          strcmp(op, "IFCONTAINSI") == 0 || strcmp(op, "WHENCONTAINSI") == 0 ||
          strcmp(op, "IFHASI") == 0 || strcmp(op, "WHENHASI") == 0 ||
    ...
    if (mode == 0) {
      ...
      if (nn == 0) take = 1;
        ...
        if (icase) take = (strncasecmp(cur, needle, nn) == 0) ? 1 : 0;
        else take = (memcmp(cur, needle, nn) == 0) ? 1 : 0;
      }
    } else if (mode == 1) {
      if (nn == 0) take = 1;
      else if (nn <= cn) {
        if (icase) take = (strncasecmp(cur + cn - nn, needle, nn) == 0) ? 1 : 0;
        else take = (memcmp(cur + cn - nn, needle, nn) == 0) ? 1 : 0;
      }
    } else if (mode == 2) {
      if (nn == 0) take = 1;
      else if (icase) {
        size_t i;
        for (i = 0; i + nn <= cn; i++) {
          if (strncasecmp(cur + i, needle, nn) == 0) { take = 1; break; }
        }
      } else if (strstr(cur, needle) != NULL) take = 1;
    } else {
      take = icase ? (strcasecmp(cur, needle) == 0 ? 1 : 0)
                   : (strcmp(cur, needle) == 0 ? 1 : 0);
    }
    body_start = *L;
    while (L->cur.kind != TK_EOF && depth > 0) {
      if (kw(&L->cur,"BREAK")||kw(&L->cur,"CONTINUE")||kw(&L->cur,"NEXT")||kw(&L->cur,"SKIP")){
    ...
    }
    {
      const char *ntag;
      if (icase)
        ntag = mode == 1 ? "IFENDSI_N" : mode == 2 ? "IFCONTAINSI_N"
              : mode == 3 ? "IFEQSI_N" : "IFSTARTSI_N";
      else
        ntag = mode == 1 ? "IFENDS_N" : mode == 2 ? "IFCONTAINS_N"
  ...
  /* IFGT name value … [ELSE …] END — body if var (num) > value.
   * IFLT / IFGE / IFLE / IFEQN — < >= <= == numeric branches.
   * Promotes string vars via strtol. Usability: retries/ports without IF n > 0 glue. */
  if (kw(&L->cur,"IFGT")||kw(&L->cur,"WHENGT")||kw(&L->cur,"IFGREATER")||
      kw(&L->cur,"IFLT")||kw(&L->cur,"WHENLT")||kw(&L->cur,"IFLESS")||
      kw(&L->cur,"IFGE")||kw(&L->cur,"WHENGE")||kw(&L->cur,"IFGTE")||
      kw(&L->cur,"IFLE")||kw(&L->cur,"WHENLE")||kw(&L->cur,"IFLTE")||
      kw(&L->cur,"IFEQN")||kw(&L->cur,"WHENEQN")||kw(&L->cur,"IFNUMEQ")||
      kw(&L->cur,"IFEQNUM")||kw(&L->cur,"IFNEE")||kw(&L->cur,"IFNEQN")||
      kw(&L->cur,"WHENNEQN")||kw(&L->cur,"IFNUMNE")){
    int aln = L->cur.line;
    int mode = 0; /* 0 >, 1 <, 2 >=, 3 <=, 4 ==, 5 != */
    int take = 0;
    char name[48], op[24];
    long lhs = 0, rhs = 0;
    Var *vv;
    ...
    if (strcmp(op, "IFLT") == 0 || strcmp(op, "WHENLT") == 0 ||
        strcmp(op, "IFLESS") == 0)
      mode = 1;
    else if (strcmp(op, "IFGE") == 0 || strcmp(op, "WHENGE") == 0 ||
             strcmp(op, "IFGTE") == 0)
      mode = 2;
    else if (strcmp(op, "IFLE") == 0 || strcmp(op, "WHENLE") == 0 ||
             strcmp(op, "IFLTE") == 0)
      mode = 3;
    else if (strcmp(op, "IFEQN") == 0 || strcmp(op, "WHENEQN") == 0 ||
             strcmp(op, "IFNUMEQ") == 0 || strcmp(op, "IFEQNUM") == 0)
      mode = 4;
    else if (strcmp(op, "IFNEE") == 0 || strcmp(op, "IFNEQN") == 0 ||
             strcmp(op, "WHENNEQN") == 0 || strcmp(op, "IFNUMNE") == 0)
      mode = 5;
    else
      mode = 0;
    lex_next(L);
    if (L->cur.kind != TK_IDENT) {
      fail(vm, "IFGT needs name value — IFGT retries 3 … END");
      return -1;
    }
    snprintf(name, sizeof name, "%s", L->cur.text);
    lex_next(L);
    if (kw(&L->cur, "THAN") || kw(&L->cur, "TO") || kw(&L->cur, "OF") ||
        kw(&L->cur, "WITH") || kw(&L->cur, "CMP"))
      lex_next(L);
    if (L->cur.kind == TK_EOF || kw(&L->cur, "THEN") || kw(&L->cur, "END") ||
        kw(&L->cur, "ELSE") || kw(&L->cur, "ELIF")) {
      fail(vm, "IFGT needs value — IFGT n 0 … END");
      return -1;
    }
    rhs = parse_expr(vm, L);
    skip_nl(L);
    ...
    if (!vv)
      ...
      char *end = NULL;
      lhs = strtol(vv->sval, &end, 10);
      if (!vv->sval[0] || end == vv->sval) lhs = 0;
    } else
      lhs = vv->val;
    if (mode == 0) take = (lhs > rhs) ? 1 : 0;
    else if (mode == 1) take = (lhs < rhs) ? 1 : 0;
    else if (mode == 2) take = (lhs >= rhs) ? 1 : 0;
    else if (mode == 3) take = (lhs <= rhs) ? 1 : 0;
    else if (mode == 4) take = (lhs == rhs) ? 1 : 0;
    else take = (lhs != rhs) ? 1 : 0;
    body_start = *L;
    while (L->cur.kind != TK_EOF && depth > 0) {
      if (kw(&L->cur,"BREAK")||kw(&L->cur,"CONTINUE")||kw(&L->cur,"NEXT")||kw(&L->cur,"SKIP")){
        lex_next(L);
      ...
      if (kw(&L->cur,"IF")||kw(&L->cur,"UNLESS")||kw(&L->cur,"IFERR")||kw(&L->cur,"IFOK")||
          ...
          kw(&L->cur,"IFEQN")||kw(&L->cur,"IFNEQN")||kw(&L->cur,"IFHASLINE")||kw(&L->cur,"IFINBAG")||kw(&L->cur,"WHENHASLINE")||kw(&L->cur,"WHENINBAG")||kw(&L->cur,"IFBETWEEN")||kw(&L->cur,"IFINRANGE")||kw(&L->cur,"IFWITHIN")||kw(&L->cur,"WHENBETWEEN")||
          kw(&L->cur,"WHENERR")||kw(&L->cur,"WHENOK")||
          kw(&L->cur,"LOOP")||kw(&L->cur,"SLOOP")||
          kw(&L->cur,"WHILE")||kw(&L->cur,"FOR")||kw(&L->cur,"EACH")||kw(&L->cur,"FN")||
          kw(&L->cur,"REPEAT")||kw(&L->cur,"TRY")||kw(&L->cur,"GUARD")||kw(&L->cur,"CASE")||
    ...
    if (depth > 1 || (depth == 1 && L->cur.kind == TK_EOF)) {
      char ebuf[160];
      snprintf(ebuf, sizeof ebuf,
               "IFGT without END line %d — IFGT name value … [ELSE …] END", aln);
      fail(vm, ebuf); return -1;
    }
    {
      const char *ntag = mode == 1 ? "IFLT_N" : mode == 2 ? "IFGE_N"
                        : mode == 3 ? "IFLE_N" : mode == 4 ? "IFEQN_N"
                        : mode == 5 ? "IFNEQN_N" : "IFGT_N";
      if (take) {
        Lex body = body_start;
        if (exec_stmts_until(vm, &body, "END", "ELSE") < 0) return -1;
        depth = 1;
  ...
  /* IFHASLINE|IFINBAG name line … [ELSE …] END — body if bag has exact field.
   * Twin of HASINBAG probe as a branch form. Empty bag / missing var → false.
   * Usability: work-queue ack without HASINBAG + IF LAST_N glue. */
  if (kw(&L->cur,"IFHASLINE")||kw(&L->cur,"WHENHASLINE")||kw(&L->cur,"IFINBAG")||
      kw(&L->cur,"WHENINBAG")||kw(&L->cur,"IFBAGHAS")||kw(&L->cur,"WHENBAGHAS")||
      kw(&L->cur,"IFHASLINEIN")||kw(&L->cur,"IFLINEIN")){
    int aln = L->cur.line;
    int take = 0;
    char name[48];
    ...
    lex_next(L);
    if (L->cur.kind != TK_IDENT && L->cur.kind != TK_STR) {
      fail(vm, "IFHASLINE needs name line — IFHASLINE work \"job\" … END");
      return -1;
    }
    snprintf(name, sizeof name, "%s", L->cur.text);
    lex_next(L);
    if (kw(&L->cur, "WITH") || kw(&L->cur, "OF") || kw(&L->cur, "FOR") ||
        kw(&L->cur, "LINE") || kw(&L->cur, "HAS") || kw(&L->cur, "CONTAINS"))
      lex_next(L);
    line[0] = 0;
    if (resolve_str_arg(vm, L, line, sizeof line) != 0) {
      fail(vm, "IFHASLINE needs line — IFHASLINE work \"job\" … END");
      return -1;
    }
    skip_nl(L);
    if (kw(&L->cur,"THEN")) { lex_next(L); skip_nl(L); }
    ...
    /* exact newline-field membership (same as HASINBAG) */
    take = 0;
    bp = bag;
    while (*bp) {
      start = bp;
    ...
    while (L->cur.kind != TK_EOF && depth > 0) {
      ...
      if (kw(&L->cur,"IF")||kw(&L->cur,"UNLESS")||kw(&L->cur,"IFERR")||kw(&L->cur,"IFOK")||
          kw(&L->cur,"IFEMPTY")||kw(&L->cur,"IFNONEMPTY")||kw(&L->cur,"IFDEFINED")||
          kw(&L->cur,"IFUNDEF")||kw(&L->cur,"IFSTARTS")||kw(&L->cur,"IFENDS")||
          kw(&L->cur,"IFCONTAINS")||kw(&L->cur,"IFHAS")||kw(&L->cur,"IFEQS")||kw(&L->cur,"IFSTARTSI")||kw(&L->cur,"IFENDSI")||kw(&L->cur,"IFCONTAINSI")||kw(&L->cur,"IFHASI")||kw(&L->cur,"IFEQSI")||
          kw(&L->cur,"IFGT")||kw(&L->cur,"IFLT")||kw(&L->cur,"IFGE")||kw(&L->cur,"IFLE")||
          kw(&L->cur,"IFEQN")||kw(&L->cur,"IFNEQN")||
          kw(&L->cur,"IFHASLINE")||kw(&L->cur,"IFINBAG")||kw(&L->cur,"WHENHASLINE")||
          kw(&L->cur,"WHENINBAG")||kw(&L->cur,"IFBETWEEN")||kw(&L->cur,"IFINRANGE")||kw(&L->cur,"IFWITHIN")||kw(&L->cur,"WHENBETWEEN")||
          kw(&L->cur,"WHENERR")||kw(&L->cur,"WHENOK")||
          kw(&L->cur,"LOOP")||kw(&L->cur,"SLOOP")||
          kw(&L->cur,"WHILE")||kw(&L->cur,"FOR")||kw(&L->cur,"EACH")||kw(&L->cur,"FN")||
          kw(&L->cur,"REPEAT")||kw(&L->cur,"TRY")||kw(&L->cur,"GUARD")||kw(&L->cur,"CASE")||
    ...
    if (depth > 1 || (depth == 1 && L->cur.kind == TK_EOF)) {
      char ebuf[160];
      snprintf(ebuf, sizeof ebuf,
               "IFHASLINE without END line %d — IFHASLINE name line … [ELSE …] END", aln);
      fail(vm, ebuf); return -1;
    }
    if (take) {
      Lex body = body_start;
      ...
      if (kw(&L->cur,"END")) lex_next(L);
      var_set_num(vm, "IFHASLINE_N", 1);
      var_set_num(vm, "IFINBAG_N", 1);
      bump(vm); return 1;
    }
    if (kw(&L->cur,"ELSE")) {
      lex_next(L);
      ...
      if (kw(&L->cur,"END")) lex_next(L);
      var_set_num(vm, "IFHASLINE_N", 0);
      var_set_num(vm, "IFINBAG_N", 0);
      bump(vm); return 1;
    }
    if (kw(&L->cur,"END")) lex_next(L);
    var_set_num(vm, "IFHASLINE_N", 0);
  ...
  /* IFBETWEEN|IFINRANGE name lo [TO] hi … [ELSE …] END — body if var in [lo,hi].
   * Inclusive; swaps lo/hi if reversed. Promotes string vars via strtol.
   * Usability: port/retry/status gates without INRANGE + IF LAST_N glue. */
  if (kw(&L->cur,"IFBETWEEN")||kw(&L->cur,"WHENBETWEEN")||kw(&L->cur,"IFINRANGE")||
      kw(&L->cur,"WHENINRANGE")||kw(&L->cur,"IFWITHIN")||kw(&L->cur,"WHENWITHIN")||
      kw(&L->cur,"IFRANGE")||kw(&L->cur,"WHENRANGE")||kw(&L->cur,"IFNUMIN")){
    int aln = L->cur.line;
    int take = 0;
    char name[48];
    long lhs = 0, lo = 0, hi = 0;
    Var *vv;
    Lex body_start;
    int depth = 1;
    lex_next(L);
    if (L->cur.kind != TK_IDENT) {
      fail(vm, "IFBETWEEN needs name lo hi — IFBETWEEN port 1 65535 … END");
      return -1;
    }
    snprintf(name, sizeof name, "%s", L->cur.text);
    lex_next(L);
    if (kw(&L->cur, "IN") || kw(&L->cur, "FROM") || kw(&L->cur, "OF") ||
        kw(&L->cur, "WITH") || kw(&L->cur, "RANGE"))
      lex_next(L);
    if (L->cur.kind == TK_EOF || kw(&L->cur, "THEN") || kw(&L->cur, "END") ||
        kw(&L->cur, "ELSE") || kw(&L->cur, "ELIF")) {
      fail(vm, "IFBETWEEN needs lo hi — IFBETWEEN n 1 10 … END");
      return -1;
    }
    lo = parse_expr(vm, L);
    if (kw(&L->cur, "TO") || kw(&L->cur, "THROUGH") || kw(&L->cur, "THRU") ||
        kw(&L->cur, "AND") || kw(&L->cur, "DOTDOT") || kw(&L->cur, ".."))
      lex_next(L);
    if (L->cur.kind == TK_EOF || kw(&L->cur, "THEN") || kw(&L->cur, "END") ||
        kw(&L->cur, "ELSE") || kw(&L->cur, "ELIF")) {
      fail(vm, "IFBETWEEN needs hi — IFBETWEEN n 1 TO 10 … END");
      return -1;
    }
    hi = parse_expr(vm, L);
    ...
    if (depth > 1 || (depth == 1 && L->cur.kind == TK_EOF)) {
      char ebuf[160];
      snprintf(ebuf, sizeof ebuf,
               "IFBETWEEN without END line %d — IFBETWEEN name lo hi … [ELSE …] END", aln);
      fail(vm, ebuf); return -1;
    }
    var_set_num(vm, "IFBETWEEN_LO", lo);
    var_set_num(vm, "IFBETWEEN_HI", hi);
    var_set_num(vm, "IFBETWEEN_X", lhs);
    if (take) {
      Lex body = body_start;
      if (exec_stmts_until(vm, &body, "END", "ELSE") < 0) return -1;
      depth = 1;
      ...
      if (kw(&L->cur,"END")) lex_next(L);
      var_set_num(vm, "IFBETWEEN_N", 1);
      var_set_num(vm, "IFINRANGE_N", 1);
      bump(vm); return 1;
    }
    if (kw(&L->cur,"ELSE")) {
      lex_next(L);
      ...
      if (kw(&L->cur,"END")) lex_next(L);
      var_set_num(vm, "IFBETWEEN_N", 0);
      var_set_num(vm, "IFINRANGE_N", 0);
      bump(vm); return 1;
    }
    if (kw(&L->cur,"END")) lex_next(L);
    var_set_num(vm, "IFBETWEEN_N", 0);
