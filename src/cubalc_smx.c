...
/* --- SMX soft-OOB mesh stability / life-force --- */

float cubalc_smx_life_force_tick(cubalc_smx_ctx *ctx, float unity_in, int stress) {
  float lf;
  if (!ctx) return 0.0f;
  if (unity_in < 0.0f) unity_in = 0.0f;
  if (unity_in > 1.0f) unity_in = 1.0f;
  lf = ctx->life_force > 0.0f ? ctx->life_force : unity_in;
  if (stress) {
    lf *= 0.92f;
  } else {
    lf = lf * 0.85f + unity_in * 0.15f;
  }
  if (ctx->hold_flash && lf < CUBALC_SMX_LIFE_FLOOR)
    lf = CUBALC_SMX_LIFE_FLOOR;
  if (lf > 1.0f) lf = 1.0f;
  if (lf < 0.0f) lf = 0.0f;
  ctx->life_force = lf;
  return lf;
...
int cubalc_smx_soft_oob(cubalc_smx_ctx *ctx, int open_rc, uint32_t peer_seq,
  ...
  cubalc_smx_mesh_pulse *p = out ? out : &local;
  memset(p, 0, sizeof(*p));
  if (!ctx) return -1;
  if (!ctx->key_ok) {
    snprintf(ctx->last_err, sizeof ctx->last_err, "soft_oob_fail_closed_no_key");
  ...
  /* only soft-heal known recoverable stress codes */
  if (open_rc == -6 /* replay_seq */ || open_rc == -7 /* compat */) {
    if (peer_seq > ctx->last_rx_seq)
      gap = peer_seq - ctx->last_rx_seq;
    else if (ctx->last_rx_seq > peer_seq)
      gap = ctx->last_rx_seq - peer_seq;
    if (gap == 0) gap = 1;
    if (gap <= CUBALC_SMX_OOB_MAX_GAP) {
      /* advance watermark to peer to clear stall; sticky hold_flash */
      if (peer_seq > ctx->last_rx_seq)
        ctx->last_rx_seq = peer_seq;
      ctx->hold_flash = 1;
      ctx->soft_repairs++;
      p->gap_healed = gap;
      p->soft_repairs = ctx->soft_repairs;
      p->oob_active = 1;
      p->life_force = cubalc_smx_life_force_tick(ctx, local_unity > 0.f ? local_unity : 0.5f, 1);
      snprintf(p->reason, sizeof p->reason,
               open_rc == -6 ? "heal_replay_gap" : "heal_compat_stress");
      ctx->last_err[0] = 0;
      return 0;
    }
    snprintf(p->reason, sizeof p->reason, "gap_too_large");
    p->life_force = cubalc_smx_life_force_tick(ctx, local_unity, 1);
    return -3;
  }
  if (open_rc == 0) {
    p->life_force = cubalc_smx_life_force_tick(ctx, local_unity > 0.f ? local_unity : 1.0f, 0);
    p->soft_repairs = ctx->soft_repairs;
    snprintf(p->reason, sizeof p->reason, "stable");
    return 0;
  }
  /* hard failures (mac/magic/no_key) stay fail-closed — no soft path */
  snprintf(p->reason, sizeof p->reason, "hard_fail_%d", open_rc);
  p->life_force = cubalc_smx_life_force_tick(ctx, local_unity, 1);
  return open_rc;
}
...
int cubalc_smx_seal_oob(cubalc_smx_ctx *ctx, const cubalc_atom *atom,
  ...
  cubalc_smx_hdr *h;
  if (!ctx) return -1;
  /* recovery frames must advance past healed rx watermark */
  if (ctx->last_tx_seq <= ctx->last_rx_seq)
    ctx->last_tx_seq = ctx->last_rx_seq;
  int rc = cubalc_smx_seal(ctx, atom, from, to, out, cap, n_out);
  if (rc != 0) return rc;
  h = (cubalc_smx_hdr *)out;
  h->flags |= CUBALC_SMX_F_SOFT_OOB | CUBALC_SMX_F_HOLD_FLASH;
  h->hold_flash = 1;
  /* re-MAC over mutated header+bits */
  {
    size_t bits_n = (atom->matrix.n + 7) / 8;
    uint8_t mac[CUBALC_SMX_MAC_LEN];
    hmac_sha256(ctx->key, CUBALC_SMX_KEY_LEN, out, sizeof(*h) + bits_n, mac);
    memcpy(out + sizeof(*h) + bits_n, mac, CUBALC_SMX_MAC_LEN);
  }
