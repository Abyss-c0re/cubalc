...
int main(int argc, char **argv) {
  ...
  if (strcmp(cmd, "forms") == 0 || strcmp(cmd, "ops") == 0 ||
    ...
    static const struct { const char *name; const char *plane; const char *hint; } forms[] = {
      ...
      {"FOR", "flow", "FOR … END"},
      {"EACH", "flow", "EACH CUBE|CELL|LINE|PROP|METHOD|OBJ … END"},
      ...
      {"MERGEP", "flow", "MERGEP overlay — multi-key overlay into PLATE"},
      {"DEFAULTP", "flow", "DEFAULTP|ENSUREP key value — set PLATE key only if missing"},
      {"TOGGLEP", "flow", "TOGGLEP key — flip PLATE flag 0↔1 · miss→1"},
      {"NEEDP", "flow", "NEEDP key… — fail-fast if PLATE missing keys"},
      {"HASP", "flow", "HASP key — soft 0|1 if PLATE has key"},
...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT — LAST_N 1 if euid==0 privilege probe"},
  ...
  if (strcmp(cmd, "init") == 0 || strcmp(cmd, "new") == 0 ||
     ...
     * cubalc init [path] [--force|-f] [--plate|--session]
    ...
    int force = 0, i, wrote = 0, existed = 0, want_plate = 0;
    char abspath[512];
    char parent[512];
    FILE *f;
...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT euid==0 privilege probe"},
    ...
    static const struct { const char *name; const char *path; const char *hint; } aliases[] = {
      ...
      {"p2p_doc", "docs/P2P_SMX.md", "binary mesh wire"},
      {"hello", "programs/hello_cube.cubalc", "minimal starter"},
      {"init", "cubalc init [--plate]", "scaffold agent_boot or plate_session starter"},
    };
