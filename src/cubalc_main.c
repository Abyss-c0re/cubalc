...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT — LAST_N 1 if euid==0 privilege probe"},
  ...
  if (strcmp(cmd, "init") == 0 || strcmp(cmd, "new") == 0 ||
    ...
    int want_fat = 0, want_fat_session = 0;
    int want_list = 0;
    char abspath[512];
    char parent[512];
    char from_body[8192];
    ...
    if (want_list) {
      ...
      printf("# run: cubalc init [path] [--plate|--peer|--fat|--fat-session|--from lib] [--force]\n");
      printf("# flag\tid\thint\n");
...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT euid==0 privilege probe"},
  ...
  if (strcmp(cmd, "help") == 0 || strcmp(cmd, "-h") == 0) {
    ...
    fprintf(stderr,
      ...
      "    init|new|scaffold [f]  --list · --plate · --peer · --fat · --fat-session · --from lib\n"
      ...
      "    picklib|newestlib|sortlibs|freshlibs|libage|hasfresh|needfresh|nthlib  filter duals\n"
