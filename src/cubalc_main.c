...
int main(int argc, char **argv) {
  ...
  if (strcmp(cmd, "forms") == 0 || strcmp(cmd, "ops") == 0 ||
    ...
    static const struct { const char *name; const char *plane; const char *hint; } forms[] = {
      ...
      {"FOR", "flow", "FOR … END"},
      {"EACH", "flow", "EACH CUBE|CELL|LINE|PROP|METHOD|OBJ|KEY … END"},
      {"EACH LINE", "flow", "EACH LINE [as name] [IN str|obj field] … END — bag/LIST walk"},
      {"EACH KEY", "flow", "EACH KEY [AS name] OF|FROM plate … END — walk plate keys · VALUE bound"},
      {"EACH KEYNEST", "flow", "EACH KEYNEST nest [FROM plate] … END — walk nested object keys · VALUE bound"},
      {"EACH KEY OF NEST", "flow", "EACH KEY OF NEST nest [FROM plate] … END — alias of KEYNEST"},
      ...
      {"SETP", "flow", "SETP [FROM plate] key value — set key on PLATE or named plate · multi-plate"},
      {"INCP", "flow", "INCP [FROM plate] key [delta] — bump numeric key · multi-plate"},
      {"DELP", "flow", "DELP [FROM plate] key — drop key · soft miss · multi-plate"},
      {"GETP", "flow", "GETP [FROM plate] key [OR fallback] — peel value · multi-plate"},
      ...
      {"HASP", "flow", "HASP [FROM plate] key — soft 0|1 presence · multi-plate"},
      ...
      {"PLUCKP", "flow", "PLUCKP|GETPALL [FROM plate] key… — multi-key peel → value bag · multi-plate"},
      ...
      {"FILLPFILE", "flow", "FILLPFILE [STRICT] [FROM plate] tmpl [out] — multi-plate materialize"},
...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT — LAST_N 1 if euid==0 privilege probe"},
  ...
  if (strcmp(cmd, "init") == 0 || strcmp(cmd, "new") == 0 ||
     ...
     * Default: agent_boot + place/plug. --plate: plate_session durable state.
    ...
    if (want_peer)
      ...
      printf("# next: cubalc run %s · cubalc cat plate_session · COOKBOOK §8\n", path);
    else
    ...
    printf("{\"schema\":\"cubalc.init.v1\",\"ok\":true,\"cmd\":\"init\","
           ...
           want_peer
               ...
               : (want_plate
               ? "INCLUDE plate_session + SETP/NEEDP/DUMPP/plate_save starter"
...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT euid==0 privilege probe"},
    ...
    static const struct { const char *name; const char *path; const char *hint; } aliases[] = {
      ...
      {"p2p_doc", "docs/P2P_SMX.md", "binary mesh wire"},
      {"hello", "programs/hello_cube.cubalc", "minimal starter"},
      {"init", "cubalc init [--plate|--peer]", "scaffold agent_boot, plate_session, or multi-plate peer"},
    };
