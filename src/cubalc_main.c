  ...
  if (strcmp(cmd, "doctor") == 0 || strcmp(cmd, "health") == 0) {
    ...
    }
    {
      ...
      printf("{\"schema\":\"cubalc.doctor.v1\",\"ok\":%s,"
             ...
             "\"INCLUDE fat_session · fat_boot+plate_boot durable nest (vars_max=%d)\","
             ...
             "\"programs/lib/fat_session.cubalc\"]"
             "}\n",
             ok ? "true" : "false",
...
{"SYS ISROOT", "host", "SYS ISROOT|AMROOT — LAST_N 1 if euid==0 privilege probe"},
  ...
  if (strcmp(cmd, "init") == 0 || strcmp(cmd, "new") == 0 ||
    ...
    if (want_list) {
      ...
      printf("# run: cubalc init [path] [--plate|--peer|--fat] [--force]\n");
      printf("# flag\tid\thint\n");
      ...
      printf("{\"schema\":\"cubalc.init.v1\",\"ok\":true,\"cmd\":\"init\","
             "\"op\":\"list\",\"n\":4,\"version\":\"%s\","
             "\"note\":\"scaffold catalog — cubalc init path --plate|--peer|--fat\","
             "\"templates\":["
