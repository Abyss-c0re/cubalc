...
int main(int argc, char **argv) {
  ...
  if (strcmp(cmd, "forms") == 0 || strcmp(cmd, "ops") == 0 ||
    ...
    static const struct { const char *name; const char *plane; const char *hint; } forms[] = {
      ...
      {"SETP", "flow", "SETP key value — set key on PLATE var · write-back · no LET glue"},
      {"INCP", "flow", "INCP key [delta] — bump numeric PLATE key · default +1"},
      {"DELP", "flow", "DELP key — drop PLATE key · soft miss"},
      {"GETP", "flow", "GETP key [OR fallback] — peel value from PLATE"},
