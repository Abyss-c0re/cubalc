 ...
 * Energy path: ascending aorta root / sinuses → aortic arch curvature →
 * brachiocephalic · left carotid · left subclavian takeoffs → descending approach →
 * systemic free-energy readiness (oxygenated arch conduit seal).
 * Pure C. No SYS glue.
 */
#include <string.h>
...
int cubalc_smx_aortic_arch_branch_takeoffs(void) {
  return 3; /* brachiocephalic, LCC, L subclavian */
}
...
int cubalc_smx_aortic_arch_selftest(void) {
  ...
  if (cubalc_smx_aortic_arch_branch_takeoffs() != 3) return 0;
  return 1;
}
