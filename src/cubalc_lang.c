...
static int parse_form(VM *vm, Lex *L){
  ...
  if (kw(&L->cur,"SYS") || kw(&L->cur,"HOST")){
    ...
    /* SYS JSON "key" [FROM src|LAST] → LAST = field (soft: OK=0 on miss) */
    /* SYS JSONONLY|ONLYP "key" [FROM …] → hard fail on miss (REQUIRE dual) */
    if (kw(&L->cur,"JSON") || kw(&L->cur,"JGET") ||
        kw(&L->cur,"JSONONLY") || kw(&L->cur,"ONLYP") || kw(&L->cur,"REQUIRE")){
      int hard = 0;
      if (kw(&L->cur,"JSONONLY") || kw(&L->cur,"ONLYP")) hard = 1;
      if (kw(&L->cur,"REQUIRE")){
        lex_next(L);
        if (kw(&L->cur,"JSON") || kw(&L->cur,"JSONONLY") || kw(&L->cur,"ONLYP") ||
            kw(&L->cur,"JGET") || kw(&L->cur,"NOEXTRAP")){
          hard = 1;
          /* REQUIRE ONLYP / REQUIRE JSON / REQUIRE NOEXTRAP */
        } else {
          fail(vm,"REQUIRE JSON|JSONONLY|ONLYP|NOEXTRAP"); return -1;
        }
      }
      lex_next(L);
      char key[96]="content";
      ...
      int from_explicit = 0;
      if (kw(&L->cur,"FROM") || kw(&L->cur,"FILE")){
        lex_next(L);
        from_explicit = 1;
        if (L->cur.kind==TK_STR){
          snprintf(srcbuf,sizeof srcbuf,"%s",L->cur.text); src=srcbuf; lex_next(L);
        } else if (L->cur.kind==TK_IDENT){
          if (strcmp(L->cur.text,"LAST")==0){ src=vm->last_str; }
          else {
            Var *v=var_get(vm,L->cur.text,0);
            if (v && v->is_str) src=v->sval;
            else { fail(vm,"SYS JSON FROM str|LAST|var"); return -1; }
          }
          lex_next(L);
        } else { fail(vm,"SYS JSON FROM str|LAST|var"); return -1; }
      }
      /* sticky multi-key plate: if LAST is a field value, reuse last object plate */
      if (!from_explicit) {
        const char *ls = vm->last_str;
        int looks_obj = 0;
        if (ls) {
          while (*ls==' '||*ls=='\t'||*ls=='\n'||*ls=='\r') ls++;
          if (*ls=='{' || *ls=='[') looks_obj = 1;
        }
        if (!looks_obj && vm->json_src[0]) src = vm->json_src;
      }
      /* cache object-looking source for successive key lookups */
      if (src) {
        const char *p = src;
        while (*p==' '||*p=='\t'||*p=='\n'||*p=='\r') p++;
        if (*p=='{' || *p=='[')
          snprintf(vm->json_src, sizeof vm->json_src, "%s", src);
      }
      if (vm->trace) fprintf(vm->trace, "# JSON key=%s src_head=%.40s json_src_head=%.40s from=%d js0=%d ls0=%d looks=%d\n",
                              key, src?src:"", vm->json_src, from_explicit,
                              vm->json_src[0]?1:0, vm->last_str[0]?1:0,
                              (src && src[0]=='{')?1:0);
      fprintf(stderr, "DBG json_src_len=%zu last_str_len=%zu src=%p last=%p js=%p\n",
              strlen(vm->json_src), strlen(vm->last_str), (void*)src, (void*)vm->last_str, (void*)vm->json_src);
      cubalc_host_result hr;
      if (cubalc_host_json_get(src, key, &hr)!=0){
        if (hard){
          char em[160];
          snprintf(em,sizeof em,"JSONONLY missing key '%s'", key);
          fail(vm, em); return -1;
        }
        var_set_str(vm,"LAST","");
        var_set_num(vm,"LAST_N",0);
    ...
    fail(vm, "SYS: READ|WRITE|ENV|EXIST|WHICH|HTTP|SPAWN|JOIN|JSON|JSONONLY|CHAT|ARG|NUM");
    return -1;
  }
  ...
  if (kw(&L->cur,"LET")){
    ...
    if (L->cur.kind==TK_STR){
      ...
      if (strcmp(name,"LAST")==0){
        snprintf(vm->last_str,sizeof vm->last_str,"%s",L->cur.text);
        vm->last_n = (long)strlen(L->cur.text);
    ...
    /* string-var RHS: LET x = other_str  (also LET LAST = payload) */
    if (L->cur.kind==TK_IDENT){
      Var *rv = var_get(vm, L->cur.text, 0);
      if (rv && rv->is_str){
        var_set_str(vm, name, rv->sval);
        if (strcmp(name,"LAST")==0){
          snprintf(vm->last_str,sizeof vm->last_str,"%s",rv->sval);
          vm->last_n = (long)strlen(rv->sval);
          var_set_num(vm,"LAST_N",vm->last_n);
        }
        lex_next(L);
        bump(vm); return 1;
    ...
    if (strcmp(name,"LAST_N")==0) vm->last_n = v;
    bump(vm); return 1;
  }
  if (kw(&L->cur,"SETBIT")){
    lex_next(L);
