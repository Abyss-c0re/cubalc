...
Shell: `cubalc plate get|set|inc|show|ensure|merge|eq|diff|changelog|has|need|pluck path.json` · `cubalc libs` · `cubalc cat plate_session`.  
Keys may be dotted: `cubalc plate get agent.json freq.error` · `plate need … host freq.error` · `plate pluck … host freq.error n`.  
`ensure`/`merge` seed+patch · `eq`/`diff`/`changelog` sync · `has`/`need` multi-key contract · `pluck` multi-key peel bag (no `.cubalc`).  
