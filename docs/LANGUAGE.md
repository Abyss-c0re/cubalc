...
Data plane: `CELLSET` `CELL(i)` `PUSH` `POP` `PEEK` `SP`/`STACKLEN` `SWAPCELL` `CLEARCELLS` `CLEARSTACK` · `DUP`/`DROP`/`SWAP`/`OVER`/`ROT`/`PICK` · `NIP`/`TUCK`/`2DUP`/`2DROP`/`2SWAP`/`ROLL`/`DEPTH` · `FILLCELL` · `COPYCELL`/`MOVECELL` · `FINDCELL`/`COUNTCELL` · `REVCELL` · `ADDCELL`/`MULCELL`/`IOTA`/`SORTCELL` · `MINIDX`/`MAXIDX`/`ROTCELL`/`SHIFTCELL` · `INC`/`DEC` · `SUMCELL`/`MINCELL`/`MAXCELL`

Math plane: `ADDMOD` `SUBMOD` `MULMOD` `POWMOD` `FIB` `ISPRIME` `IDIV` `IMOD` · `ILOG2`/`LOG2` `CTZ` `CLZ` `ISPOW2` `POW2` `NDIGITS` `DIGSUM` `MODINV`
...
Default status token: `C3`. Share: `smx`. Hold: `1`. Version: `1.12.6-universal`.
