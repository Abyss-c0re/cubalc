...
Data plane: `CELLSET` `CELL(i)` `PUSH` `POP` `PEEK` `SP`/`STACKLEN` `SWAPCELL` `CLEARCELLS` `CLEARSTACK` · `DUP`/`DROP`/`SWAP`/`OVER`/`ROT`/`PICK` · `NIP`/`TUCK`/`2DUP`/`2DROP`/`2SWAP`/`ROLL`/`DEPTH` · `ADD`/`SUB`/`MUL`/`DIV`/`MOD`/`SNEG`/`SABS` · `SAND`/`SOR`/`SXOR`/`SNOT`/`SSHL`/`SSHR` · `SEQ`/`SNE`/`SLT`/`SGT`/`SLE`/`SGE`/`SMIN`/`SMAX` · `TOCELL`/`FROMCELL` · `FILLCELL` · `COPYCELL`/`MOVECELL` · `FINDCELL`/`COUNTCELL` · `REVCELL` · `ADDCELL`/`MULCELL`/`IOTA`/`SORTCELL` · `MINIDX`/`MAXIDX`/`ROTCELL`/`SHIFTCELL` · `ANDCELL`/`ORCELL`/`XORCELL`/`NOTCELL`/`EQCELL` · `INC`/`DEC` · `SUMCELL`/`MINCELL`/`MAXCELL`
